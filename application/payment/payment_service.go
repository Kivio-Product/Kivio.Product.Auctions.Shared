...
func (s *paymentService) ProcessPaymentForCustomer(ctx context.Context, orders []*orderDomain.Order) error {
	...
	canProcess, err := s.CanProcessPaymentForCustomer(ctx, customerId)
	if err != nil {
		return fmt.Errorf("error checking payment eligibility for customer %s: %w", customerId, err)
	}

	if !canProcess {
		return fmt.Errorf("customer %s already has an approved payment", customerId)
	}
